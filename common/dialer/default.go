...
func NewDefault(router adapter.Router, options option.DialerOptions) (*DefaultDialer, error) {
	...
	if options.RoutingMark != 0 {
		dialer.Control = control.Append(dialer.Control, control.RoutingMark(int(options.RoutingMark)))
		listener.Control = control.Append(listener.Control, control.RoutingMark(int(options.RoutingMark)))
